...
bool Texture::LoadTextureA() //LE diceque la imagen tiene canal alfa
{
	...
	if (!texData) //Text Data es un arreglo de imagen que open gl si puede procesar
	{
		printf("No se encontró el archivo: %s", fileLocation);
	}
	//glGen como se aplica al textura 
	//PAra el texturizado, existen los ejes t y V
	...
	//La bandera de wrap es como se aplica la textura en los ejes 
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);// eje S paralelo a X, repetir sobre el eje
	//Si el objeto es mas grande, se ve mas veces repetido
...
{	//UnitTexture, donde cero es el indice, pasa cuando un objetivo tiene mas de in textura
